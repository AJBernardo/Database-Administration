<?php
include('navbar.php'); //IMPLEMENTED SWITCHING USERS

$requesterID = $_SESSION['requesterID'];
$toastMessage = '';

// ORIGINAL QUERY BUT HAD TO CHANGE TO CONFORM TO MARIADB SYNTAX
//SELECT DISTINCT userInfo.userInfoID, userInfo.userID, userInfo.username, userInfo.firstName, userInfo.lastName, userInfo.profilePicture, userInfo.bio, cities.cityName, provinces.provinceName, friends.requesterID, friends.requesteeID, friends.status FROM userInfo LEFT JOIN addresses ON userInfo.addressID = addresses.addressID LEFT JOIN cities ON addresses.cityID = cities.cityID LEFT JOIN provinces ON addresses.provinceID = provinces.provinceID LEFT JOIN friends ON (friends.requesteeID = userInfo.userID OR friends.requesterID = userInfo.userID) WHERE NOT (userInfoID = 10) AND NOT(requesteeID = 10 AND status = 'requested') AND NOT (requesterID = 10 AND status = 'accepted') AND NOT IN (SELECT * FROM userInfo LEFT JOIN friends ON (friends.requesterID = userInfo.userInfoID OR friends.requesteeID = userInfo.userInfoID) WHERE (status = 'accepted' AND (requesterID = 10 OR requesteeID = 10))AND NOT userInfo.userInfoID = 10;)
$displayProfileQuery = "SELECT DISTINCT 
    userInfo.userInfoID, 
    userInfo.userID, 
    userInfo.username, 
    userInfo.firstName, 
...
LEFT JOIN 
    friends ON (friends.requesteeID = userInfo.userID OR friends.requesterID = userInfo.userID)
WHERE 
	(NOT userInfo.userInfoID = $requesterID)
    AND ((NOT(friends.requesteeID = $requesterID AND friends.status = 'requested')) OR friends.status IS NULL)
    AND userInfo.userInfoID NOT IN (
        SELECT 
            CASE 
                WHEN friends.requesterID = $requesterID THEN friends.requesteeID
        ...
        WHERE 
            ...
            AND (friends.requesterID = $requesterID OR friends.requesteeID = $requesterID)
    ) GROUP BY userInfoID ORDER BY userInfoID;";
...
if (isset($_POST['btnAddFriend'])) {
  ...
  if (mysqli_num_rows($showRequest) > 0) {
    ...
    if ($existingRequest['status'] == 'requested') {
      $cancelRequestQuery = "UPDATE `friends` SET `status` = 'cancelled' WHERE (`requesterID` = '$requesterID' AND `requesteeID` = '$requesteeID' AND `status` = 'requested') OR (`requesterID` = '$requesteeID' AND `requesteeID` = '$requesterID' AND `status` = 'requested');";
            ...
            <div class="card m-auto shadow">
              ...
              <div class="card-content container-fluid">
                ...
                <form method="POST" action="">
                  ...
                  <button type="submit" class="btn" name="btnAddFriend"
